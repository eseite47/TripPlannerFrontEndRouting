...
if(hash){
  fetch("/api/itineraries/" + hash.substring(1))
  .then(result => result.json())
  .then(function(data){
    data.hotels.forEach(function(element){
      console.log(element);
      buildAttractionAssets('hotels', element)
    });
    data.restaurants.forEach(function(element){
      console.log(element);
      buildAttractionAssets('restaurants', element)
    });
    data.activities.forEach(function(element){
      console.log(element);
      buildAttractionAssets('activities', element)
    });
  })
}
...
document.getElementById('save').addEventListener("click", function(){
  return fetch("/api/itineraries", {
    ...
    body: JSON.stringify({
      hotels: state.hotels[0].id,
      restaurants: state.restaurants[0].id,
      activities: state.activities[0].id
    })
  })
